...
function getHandler(url, response){
    if (url.split("?")[0] == "/pokemon.html") getPokemonInfo(url, response);
...
async function getPokemonInfo(url, response){
    var params = url.split("?")[1];
    var id = parseInt(params.split("=")[1]);
    var content = await fs.readFile(root+url.split("?")[0], "utf8");
    db.get("SELECT * FROM POKEDEX WHERE ID = ?", id, function(err, row){
        if (err) {
